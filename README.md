...
figures/           five figures over the corpus, as PDF and PNG
...
[`figures/`](figures/) holds five figures over the corpus, in PDF and PNG,
regenerated by `scripts/make_figures.py`. Two are worth pointing at:

- `extraction-quality` — the evidence for the text-extraction argument above,
  in one plot: every judgment scored on words against numbers, with both
  thresholds drawn and the four files that fail only on numbers named.
- `corpus-coverage` — what share of each variable the two sources actually
  state. It is the fastest way to see what this dataset can and cannot answer.

[`figures/README.md`](figures/README.md) lists them all and records the drawing
conventions.
